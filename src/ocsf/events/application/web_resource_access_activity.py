...
class WebResourceAccessActivity(Application):
    ...
    @classmethod
    def with_profile(cls, profile: str) -> type["WebResourceAccessActivity"]:
        if profile == "network_proxy":
            return cast(
                type[WebResourceAccessActivity],
                create_model(
                    "WebResourceAccessActivityWithNetworkProxy",
                    __base__=(WebResourceAccessActivity, NetworkProxy_),
                ),
            )
        raise ValueError(f"Profile '{profile}' not available for WebResourceAccessActivity")
