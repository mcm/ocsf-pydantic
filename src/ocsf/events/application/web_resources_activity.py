...
from ocsf.profiles.network_proxy import NetworkProxy
...
class WebResourcesActivity(Application):
    ...
    @classmethod
    def with_profile(cls, profile: str) -> type["WebResourcesActivity"]:
        if profile == "network_proxy":
            return cast(
                type[WebResourcesActivity],
                create_model(
                    "WebResourcesActivityWithNetworkProxy", __base__=(WebResourcesActivity, NetworkProxy)
                ),
            )
        raise ValueError(f"Profile '{profile}' not available for WebResourcesActivity")
